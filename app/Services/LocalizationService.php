...
class LocalizationService
{
    ...
    public static function getLaravelLocalizationLocales(): array
    {
        return array_merge(...array_map(fn (string $value) => [$value => self::$locales[$value]], explode(',', config('app.supported_locales'))));
    }

    public static function getCrudLocales(): array
    {
        return array_map(fn (array $value) => $value['name'], self::getLaravelLocalizationLocales());
    }

    public static function supportsMultipleLocales(): bool
    {
        return count(explode(',', config('app.supported_locales'))) > 1;
    }

    public static function prepareTempTranslatableColumn(null|string|array $value): null|string
    {
        return $value ? json_encode([config('app.locale') => $value]) : null;
    }

    public static function prepareTranslatableExtrasColumn(stdClass $entity, array $extrasTranslatableKeys): ?array
    {
        $extras = $entity->extras ? json_decode($entity->extras, true) : null;
        $extrasTranslatable = [];

        foreach ($extrasTranslatableKeys as $extrasTranslatableKey) {
            if (!isset($extras[$extrasTranslatableKey])) {
                continue;
            }

            $extrasTranslatable[config('app.locale')][$extrasTranslatableKey] = $extras[$extrasTranslatableKey];
            unset($extras[$extrasTranslatableKey]);
        }

        return [
            empty($extras) ? null : json_encode($extras),
            empty($extrasTranslatable) ? null : json_encode($extrasTranslatable),
        ];
    }

    public static function getBackOfficeUrl(string $url): string
    {
        if (!self::supportsMultipleLocales() || App::getLocale() === config('app.fallback_locale')) {
            return url($url);
        }

        return url($url . '?_locale=' . App::getLocale());
    }

    public static function getAppLocaleNativeName(): string
    {
        return ucfirst(self::$locales[App::getLocale()]['native'])
            ?? ucfirst(self::$locales[config('app.fallback_locale')]['native']);
    }

    /**
     * Get the SQL expression to extract translated value from a JSON column for the specified locale.
     */
    public static function getTranslatedColumnSql(string $table, string $column, ?string $locale = null): string
    {
        ...
        return match ($driver) {
            'mysql' => "JSON_UNQUOTE(JSON_EXTRACT({$fullColumn}, '$.{$locale}'))",
            'pgsql' => "{$fullColumn}->>'{$locale}'",
            'sqlite' => "JSON_EXTRACT({$fullColumn}, '$.{$locale}')",
            default => "JSON_UNQUOTE(JSON_EXTRACT({$fullColumn}, '$.{$locale}'))", // fallback to MySQL syntax
        };
    }
}
